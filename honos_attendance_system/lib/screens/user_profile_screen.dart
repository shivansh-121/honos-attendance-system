...
class _UserProfileScreenState extends ConsumerState<UserProfileScreen> {
  ...
  void _requestEditAccess(AppUser user) async {
    if (user.role == 'admin' || user.isEditableBySupervisor == true) {
      ...
      if (user.role != 'admin' && user.isEditableBySupervisor) {
        if (user.role == 'guard') {
          await ref.read(dbProvider).updateGuardField(user.id, {'isEditableBySupervisor': false});
        } else {
          await ref.read(dbProvider).updateUserField(user.id, {'isEditableBySupervisor': false});
        }
        ...
        if (result != null && result.isNotEmpty) {
           final notif = AppNotification(
              ...
              message: '${user.name} (${user.role}) updated their profile:\n\n${result.join('\n')}',
              supervisorId: 'admin',
              guardId: user.id,
  ...
  Widget build(BuildContext context) {
    ...
    if (localUser == null) return const Scaffold(body: Center(child: CircularProgressIndicator()));
    ...
    final foundGuard = guardsAsync.value?.where((g) => g.id == localUser.id).firstOrNull;
    if (foundGuard != null) {
      user = AppUser(
        id: foundGuard.id, name: foundGuard.name, username: foundGuard.phone, password: null,
        role: 'guard', siteId: foundGuard.siteId, salary: foundGuard.salary, empId: foundGuard.empId, phone: foundGuard.phone,
        dob: foundGuard.dob, address: foundGuard.address, aadharNo: foundGuard.aadharNo, aadharPhoto: foundGuard.aadharPhoto,
        uanNo: foundGuard.uanNo, bankName: foundGuard.bankName, accountNo: foundGuard.accountNo, ifsc: foundGuard.ifsc, branch: foundGuard.branch, passbookPhoto: foundGuard.passbookPhoto, photo: foundGuard.photo, joinDate: foundGuard.joinDate, status: foundGuard.status,
        isEditableBySupervisor: foundGuard.isEditableBySupervisor,
      );
    } else {
      user = usersAsync.value?.where((u) => u.id == localUser.id).firstOrNull ?? localUser;
    }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Advances & Payroll
                  if (user.role != 'admin' && user.role != 'office_employee') ...[
                    advancesAsync.when(
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Icon(Icons.camera_alt, size: 16, color: context.colors.bgBase),
                    ),
                  ],
                ),
              ),
            const SizedBox(height: 16),
            Text(user.name, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: context.colors.txtPrimary)),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Divider(color: context.colors.txtPrimary.withValues(alpha: 0.1), height: 24),
  ...
  Widget _row(IconData icon, String label, String value) {
    return Row(
      ...
      children: [
        ...
        Expanded(flex: 3, child: Text(value.isEmpty ? '--' : value, style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.w500))),
      ],
    );
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: context.colors.bgElevated, borderRadius: BorderRadius.circular(8)),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        IconButton(icon: Icon(Icons.chevron_left, color: context.colors.txtPrimary), onPressed: () => setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month - 1))),
                        Text(DateFormat('MMM yyyy').format(_selectedMonth), style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.bold)),
                        IconButton(icon: Icon(Icons.chevron_right, color: context.colors.txtPrimary), onPressed: () => setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month + 1))),
                      ],
                    )
                  ],
                ),
                ...
                GridView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    ...
                    final attRecord = supAttendance.where((a) => a.date == dateStr).toList();
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          border: _selectedMonth.year == now.year && _selectedMonth.month == now.month && day == now.day 
                              ? Border.all(color: context.colors.txtPrimary, width: 2)
                              : null,
                        ),
                        child: Center(
                          child: Text(
