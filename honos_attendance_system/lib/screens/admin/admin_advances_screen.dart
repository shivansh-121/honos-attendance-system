  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text('Give Advance', style: TextStyle(color: context.colors.bgBase, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              title: Text('Advances & Salary', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 20, letterSpacing: -0.5)),
              background: Stack(
                fit: StackFit.expand,
                children: [
                  Container(
                  ...
                  Positioned(
                    ...
                    child: Transform.rotate(
                      angle: -0.15,
                      child: Icon(Icons.account_balance_wallet_rounded, size: 140, color: Colors.white.withValues(alpha: 0.15)),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        IconButton(
                          icon: Icon(Icons.chevron_left_rounded, color: context.colors.txtPrimary),
                          onPressed: () => setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month - 1)),
                        ),
                        Text(
                          DateFormat('MMMM yyyy').format(_selectedMonth),
                          style: TextStyle(color: context.colors.txtPrimary, fontSize: 16, fontWeight: FontWeight.bold),
                        ),
                        IconButton(
                          icon: Icon(Icons.chevron_right_rounded, color: context.colors.txtPrimary),
                          onPressed: () => setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month + 1)),
                        ),
                      ],
                    ),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: context.colors.bord.withValues(alpha: 0.3)),
                    ),
                    child: Row(
                      children: [
          ...
          advancesAsync.when(
            ...
            error: (e, st) => SliverFillRemaining(child: Center(child: Text('Error: $e', style: TextStyle(color: context.colors.red)))),
            data: (advances) {
              ...
              if (_selectedTab == 'guard') {
                return guardsAsync.when(
                  loading: () => const SliverFillRemaining(child: Center(child: CircularProgressIndicator())),
                  error: (e, st) => SliverFillRemaining(child: Center(child: Text('Error loading guards', style: TextStyle(color: context.colors.red)))),
                ...
                return usersAsync.when(
                  loading: () => const SliverFillRemaining(child: Center(child: CircularProgressIndicator())),
                  error: (e, st) => SliverFillRemaining(child: Center(child: Text('Error loading $_selectedTab', style: TextStyle(color: context.colors.red)))),
                  data: (users) {
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: isSelected ? [BoxShadow(color: context.colors.primary.withValues(alpha: 0.3), blurRadius: 8, offset: const Offset(0, 2))] : [],
          ),
      ...
      return SliverFillRemaining(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              Icon(Icons.search_off_rounded, size: 80, color: context.colors.txtMuted.withValues(alpha: 0.3)).animate().scale(delay: 200.ms, duration: 400.ms, curve: Curves.easeOutBack),
              const SizedBox(height: 20),
              Text('No $userType found', style: TextStyle(color: context.colors.txtSec, fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
        ),
      );
          ...
          (context, index) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: context.colors.bord.withValues(alpha: 0.5)),
                boxShadow: [
                  BoxShadow(color: context.colors.primary.withValues(alpha: 0.03), blurRadius: 20, offset: const Offset(0, 8)),
                ],
              ),
              ...
              child: ExpansionTile(
                ...
                shape: const Border(),
                title: Text(u.name, style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: -0.3)),
                subtitle: Padding(
                  padding: const EdgeInsets.only(top: 6),
                  child: Text('Net Pay: INR ${netPay.toStringAsFixed(0)}', style: TextStyle(color: context.colors.green, fontWeight: FontWeight.w600, fontSize: 13)),
                ),
                trailing: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                  decoration: BoxDecoration(
                    color: totalAdvance > 0 ? context.colors.red.withValues(alpha: 0.1) : context.colors.bgBase,
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Text(
                    'Adv: ${totalAdvance.toStringAsFixed(0)}', 
                    style: TextStyle(color: totalAdvance > 0 ? context.colors.red : context.colors.txtMuted, fontWeight: FontWeight.bold, fontSize: 12)
                  ),
                ),
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Row(
                              children: [
                                Icon(Icons.account_balance_wallet_rounded, size: 16, color: context.colors.txtMuted),
                                const SizedBox(width: 8),
                                Text('Fixed Monthly Salary:', style: TextStyle(color: context.colors.txtSec, fontSize: 13)),
                              ],
                            ),
                            Text('INR ${u.salary.toStringAsFixed(0)}', style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const SizedBox(height: 20),
                        Text('Advances Taken', style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 12),
                        if (userAdvances.isEmpty)
                          Container(
                            padding: const EdgeInsets.all(12),
                            decoration: BoxDecoration(color: context.colors.bgSurface, borderRadius: BorderRadius.circular(12)),
                            child: Text('No advances taken this month.', style: TextStyle(color: context.colors.txtMuted, fontStyle: FontStyle.italic)),
                          )
                        else
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(DateFormat('dd MMM yyyy').format(DateTime.parse(a.date)), style: TextStyle(color: context.colors.txtPrimary, fontSize: 13, fontWeight: FontWeight.w600)),
  ...
  void _showAddAdvanceDialog(List<Guard> guards, List<AppUser> allStaff) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Give Advance', style: TextStyle(color: context.colors.txtPrimary)),
        content: StatefulBuilder(
          builder: (ctx, setSt) {
            List<DropdownMenuItem<String>> items = [];
            if (selectedType == 'guard') {
              items = guards.map((g) => DropdownMenuItem(value: g.id, child: Text(g.name, style: TextStyle(color: context.colors.txtPrimary, fontSize: 14)))).toList();
            } else {
              items = allStaff.where((s) => s.role == selectedType).map((s) => DropdownMenuItem(value: s.id, child: Text(s.name, style: TextStyle(color: context.colors.txtPrimary, fontSize: 14)))).toList();
            }
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text('Guard', style: TextStyle(color: context.colors.txtPrimary)),
                      Radio<String>(
                        value: 'supervisor',
                        groupValue: selectedType,
                        activeColor: context.colors.primary,
                      ...
                      Text('Supervisor', style: TextStyle(color: context.colors.txtPrimary)),
                    ],
                  ),
                  Row(
                    children: [
                      ...
                      Text('Executive', style: TextStyle(color: context.colors.txtPrimary)),
                    ],
                  ),
                  const SizedBox(height: 12),
