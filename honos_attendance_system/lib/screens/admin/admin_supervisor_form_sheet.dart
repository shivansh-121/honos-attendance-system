...
class AdminSupervisorFormSheet extends StatefulWidget {
  ...
  @override
  State<AdminSupervisorFormSheet> createState() =>
      _AdminSupervisorFormSheetState();
}
...
class _AdminSupervisorFormSheetState extends State<AdminSupervisorFormSheet> {
  ...
  void initState() {
    ...
    _salary = TextEditingController(
        text: u != null ? u.salary.toStringAsFixed(0) : '');
  ...
  Future<void> _pickImage(String type) async {
    ...
    final xfile =
        await picker.pickImage(source: src, imageQuality: 50, maxWidth: 800);
    ...
    showDialog(
        ...
        builder: (ctx) => AlertDialog(
              ...
              title: Text('Error',
                  style: TextStyle(color: context.colors.txtPrimary)),
              content:
                  Text(msg, style: TextStyle(color: context.colors.txtSec)),
              actions: [
                TextButton(
  ...
  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) {
      _showError(
          'Please fill all required fields correctly. Scroll up to see the errors in red.');
      return;
    }
    ...
    try {
      ...
      if (_customSiteLoc != null) {
        if (widget.existing == null) {
          ...
          final oldSite =
              widget.allSites.firstWhere((s) => s.id == finalSiteId);
          final newSite = Site(
            id: oldSite.id,
            name: oldSite.name,
            address: _customSiteLoc!['address'],
      ...
      if (_password.text.isNotEmpty) {
        finalPassword =
            sha256.convert(utf8.encode(_password.text.trim())).toString();
      }
      ...
      if (widget.existing?.role == 'guard' || widget.role == 'guard') {
        final g = Guard(
          ...
          siteId: finalSiteId,
          supervisorId: widget.existing?.role == 'guard'
              ? 'admin'
              : 'admin', // Placeholder or existing
          ...
          joinDate:
              widget.existing?.joinDate ?? DateTime.now().toIso8601String(),
          ...
          child: ListView(
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(
                    widget.existing == null
                        ? 'Add New ${widget.role == 'executive' ? 'Executive' : 'Supervisor'}'
                        : 'Edit ${widget.role == 'executive' ? 'Executive' : 'Supervisor'}',
                    style: const TextStyle(
                        fontSize: 20,
                        fontWeight: FontWeight.bold,
                        color: Colors.white)),
                IconButton(
                    icon: Icon(Icons.close, color: context.colors.txtSec),
                    onPressed: () => Navigator.pop(context)),
              ]),
              if (widget.existing?.empId.isNotEmpty == true) ...[
                const SizedBox(height: 4),
                Text('Employee ID: ${widget.existing!.empId}',
                    style: TextStyle(
                ...
                child: GestureDetector(
                  ...
                  child: Stack(children: [
                    CircleAvatar(
                      ...
                      backgroundImage: _photoBytes.length > 200
                          ? MemoryImage(base64Decode(_photoBytes))
                          : null,
                      child: _photoBytes.length < 200
              ...
              TextFormField(
                  ...
                  style: TextStyle(color: context.colors.txtPrimary),
                  decoration: _field(
                      'Password ${widget.existing == null ? '*' : '(Leave empty to keep)'}',
                      prefix: const Icon(Icons.password, size: 18))),
              ...
              TextFormField(
                  ...
                  maxLength: 12,
                  validator: (v) => v!.length < 12 ? 'Invalid' : null),
              const SizedBox(height: 10),
              TextFormField(
                  controller: _uan,
                  style: TextStyle(color: context.colors.txtPrimary),
              ...
              OutlinedButton.icon(
                ...
                label: Text(_aadhaarBytes.isEmpty
                    ? 'Upload Aadhaar Photo'
                    : 'Aadhaar Photo Attached'),
                style: OutlinedButton.styleFrom(
                    foregroundColor: _aadhaarBytes.isEmpty
              ...
              TextFormField(
                  controller: _branch,
                  style: TextStyle(color: context.colors.txtPrimary),
                  decoration: _field('Branch Name',
                      prefix: const Icon(Icons.location_city, size: 18))),
              const SizedBox(height: 10),
              OutlinedButton.icon(
                icon: const Icon(Icons.upload_file),
                label: Text(_passbookBytes.isEmpty
                    ? 'Upload Passbook Photo'
                    : 'Passbook Photo Attached'),
                style: OutlinedButton.styleFrom(
                    foregroundColor: _passbookBytes.isEmpty
                        ? context.colors.txtSec
                        : context.colors.green),
              ...
              if (widget.allSites.isNotEmpty) ...[
                Text('Assign to Site *',
                    style:
                        TextStyle(color: context.colors.txtSec, fontSize: 13)),
                const SizedBox(height: 6),
                DropdownButtonFormField<String>(
                  initialValue: _selectedSiteId,
                  dropdownColor: context.colors.bgSurface,
                  style: TextStyle(color: context.colors.txtPrimary),
                  items: widget.allSites
                      .map((s) =>
                          DropdownMenuItem(value: s.id, child: Text(s.name)))
                      .toList(),
                  onChanged: (val) => setState(() => _selectedSiteId = val!),
                  decoration: _field('Site *',
                      prefix: const Icon(Icons.location_on, size: 18)),
                  validator: (v) =>
                      widget.role != 'admin' && (v == null || v.isEmpty)
                          ? 'Site required'
                          : null,
                ),
              ],
              ...
              OutlinedButton.icon(
                ...
                label: Text(_customSiteLoc == null
                    ? '📍 Override Site Location on Map'
              ...
              ElevatedButton.icon(
                ...
                label: Text(widget.existing == null
                    ? 'Add ${widget.role == 'executive' ? 'Executive' : 'Supervisor'}'
                    : 'Save Changes'),
