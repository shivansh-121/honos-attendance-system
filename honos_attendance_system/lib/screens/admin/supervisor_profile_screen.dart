...
class _SupervisorProfileScreenState extends ConsumerState<SupervisorProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('${s.role[0].toUpperCase()}${s.role.substring(1)} Profile', style: const TextStyle(fontWeight: FontWeight.bold)),
      ),
        ...
        child: Column(
          children: [
            CircleAvatar(
              ...
              child: s.photo.length < 200 ? Icon(Icons.person, size: 50, color: context.colors.txtMuted) : null,
            ),
            const SizedBox(height: 16),
            Text(s.name, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: context.colors.txtPrimary)),
            if (s.empId.isNotEmpty) ...[
              const SizedBox(height: 4),
              Text('Emp ID: ${s.empId}', style: TextStyle(fontSize: 14, color: context.colors.primary, fontWeight: FontWeight.w600)),
            ],
            const SizedBox(height: 4),
            Container(
              ...
              child: Text(s.role.toUpperCase(), style: TextStyle(color: context.colors.primary, fontWeight: FontWeight.bold, fontSize: 12)),
            ),
            ...
            Row(
              ...
              children: [
                ...
                Text(s.phone.isNotEmpty ? s.phone : 'No Phone', style: TextStyle(color: context.colors.txtSec)),
                const SizedBox(width: 16),
                Icon(Icons.calendar_today, size: 16, color: context.colors.txtSec),
                const SizedBox(width: 4),
                Text('Joined: ${s.joinDate.split('T').first}', style: TextStyle(color: context.colors.txtSec)),
              ],
            )
          ],
        ),
  ...
  Widget _buildAdvancesCard(AppUser s, List<Advance> allAdvances, List<Leave> allLeaves) {
    final supAdvances = allAdvances.where((a) => a.userId == s.id && (a.userType == 'supervisor' || a.userType == 'executive' || a.userType == 'employee')).toList();
    ...
    // Deduct leaves
    final myLeaves = allLeaves.where((l) => l.employeeId == s.id && l.status == 'approved').toList();
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      ...
                      if (monthLeavesCount > 0)
                        Text('-$monthLeavesCount Leave(s)', style: TextStyle(color: context.colors.red, fontSize: 10)),
                    ]),
                ],
              ),
            ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(monthName, style: TextStyle(color: context.colors.txtPrimary, fontWeight: FontWeight.bold)),
