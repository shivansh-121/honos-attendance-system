...
class _AdminGuardsManagementScreenState
  ...
  void _openGuardForm(BuildContext context, DbService db, List<Site> sites,
      {Guard? existing}) {
    if (sites.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please add a Site first!')));
      return;
    }
                      ...
                      Expanded(
                        child: sitesAsync.when(
                          data: (sites) {
                            ...
                            final safeSiteFilter =
                                (siteIds.contains(_selectedSiteFilter))
                                    ? _selectedSiteFilter
                                    : null;
                            return Container(
                      ...
                      Expanded(
                        child: usersAsync.when(
                          data: (users) {
                            ...
                            final safeSupFilter =
                                (supIds.contains(_selectedSupervisorFilter))
                                    ? _selectedSupervisorFilter
                                    : null;
                            return Container(
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 12, vertical: 2),
                              ...
                              child: DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  ...
                                  items: [
                                    const DropdownMenuItem<String>(
                                        value: null, child: Text('All Sups')),
                                    ...supervisors.map((s) => DropdownMenuItem(
                                        value: s.id,
                                        child: Text(s.name,
                                            overflow: TextOverflow.ellipsis)))
                                  ],
                                  onChanged: (val) => setState(
                                      () => _selectedSupervisorFilter = val),
                                ),
                              ),
                            );
                          },
          ...
          guardsAsync.when(
            data: (guards) {
              ...
              final filtered = guards.where((g) {
                final matchQuery = g.name
                        .toLowerCase()
                        .contains(_searchQuery.toLowerCase()) ||
                    g.empId.toLowerCase().contains(_searchQuery.toLowerCase());
                final matchSite = _selectedSiteFilter == null ||
                    g.siteId == _selectedSiteFilter;
                final matchSupervisor = _selectedSupervisorFilter == null ||
                    g.supervisorId == _selectedSupervisorFilter;
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Try adjusting your search or filters.',
                            style: TextStyle(
                                color: context.colors.txtMuted, fontSize: 14)),
                      ],
                    ),
                  ),
                );
                    ...
                    (context, index) {
                      ...
                      return GestureDetector(
                        onTap: () => Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (_) => GuardProfileScreen(guard: g))),
                        child: Container(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    icon: Icon(Icons.edit_rounded,
                                        color: context.colors.txtSec, size: 22),
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text('₹${g.salary.toStringAsFixed(0)}/mo',
                                          style: TextStyle(
                                              color: context.colors.txtSec,
                                              fontSize: 13,
                                              fontWeight: FontWeight.w600)),
...
class _AdminGuardFormSheetState extends State<_AdminGuardFormSheet> {
  ...
  void initState() {
    ...
    _selectedSiteId = g?.siteId ??
        (widget.allSites.isNotEmpty ? widget.allSites.first.id : '');

    // Safety check in case the site was deleted
    if (!widget.allSites.any((s) => s.id == _selectedSiteId) &&
        widget.allSites.isNotEmpty) {
  ...
  Future<void> _save() async {
    ...
    final selectedSite =
        widget.allSites.firstWhere((s) => s.id == _selectedSiteId);
            ...
            children: [
              ...
              if (widget.existing != null)
                Padding(
                  ...
                  child: Text(
                      'Changing the site transfers the guard to a new supervisor.',
                      style: TextStyle(
                          color: context.colors.txtMuted, fontSize: 11),
                      textAlign: TextAlign.center),
                ),
