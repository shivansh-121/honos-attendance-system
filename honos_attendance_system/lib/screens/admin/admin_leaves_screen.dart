        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              title: Text('Leave Requests', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 20, letterSpacing: -0.5)),
              background: Stack(
                fit: StackFit.expand,
                children: [
                  Container(
                  ...
                  Positioned(
                    ...
                    child: Transform.rotate(
                      angle: 0.2,
                      child: Icon(Icons.event_note, size: 140, color: Colors.white.withValues(alpha: 0.15)),
                ...
                return SliverFillRemaining(
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.event_available_rounded, size: 80, color: context.colors.txtMuted.withValues(alpha: 0.3)).animate().scale(delay: 200.ms, duration: 400.ms, curve: Curves.easeOutBack),
                        const SizedBox(height: 20),
                        Text('No leave requests.', style: TextStyle(color: context.colors.txtSec, fontSize: 18, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                );
                    ...
                    (context, index) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(color: statusColor.withValues(alpha: 0.05), blurRadius: 20, offset: const Offset(0, 8)),
                          ],
                        ),
                        child: Column(
                          ...
                          children: [
                                ...
                                Expanded(
                                  child: Row(
                                    children: [
                                      ...
                                      Expanded(child: Text(leave.employeeName, overflow: TextOverflow.ellipsis, style: TextStyle(color: context.colors.txtPrimary, fontSize: 18, fontWeight: FontWeight.bold))),
                                    ],
                                  ),
                                ),
                                Row(
                                  children: [
                                    Container(
                                      ...
                                      child: Text(leave.status.toUpperCase(), style: TextStyle(color: statusColor, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                                    ),
                                    const SizedBox(width: 8),
                                    IconButton(
                                      icon: Icon(Icons.delete, color: context.colors.red.withValues(alpha: 0.7), size: 20),
                                      ...
                                      onPressed: () {
                                        showDialog(
                                          ...
                                          builder: (ctx) => AlertDialog(
                                            ...
                                            title: Text('Delete Leave', style: TextStyle(color: context.colors.txtPrimary)),
                                            content: Text('Are you sure you want to delete this leave request?', style: TextStyle(color: context.colors.txtSec)),
                                            actions: [
                                              TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
                                              TextButton(
                                                onPressed: () {
                                                  ref.read(dbProvider).deleteLeave(leave.id);
                                                  Navigator.pop(ctx);
                                                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Leave deleted'), backgroundColor: context.colors.green));
                                                },
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Icon(Icons.date_range, color: context.colors.primary, size: 20),
                                  const SizedBox(width: 12),
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text('Leave Duration', style: TextStyle(color: context.colors.txtMuted, fontSize: 11, fontWeight: FontWeight.bold)),
                                        const SizedBox(height: 2),
                                        Text('${leave.fromDate} to ${leave.toDate}', style: TextStyle(color: context.colors.primary, fontWeight: FontWeight.w600, fontSize: 13)),
                                      ],
                                    ),
                                  ),
                                ],
                            ...
                            const SizedBox(height: 16),
                            Text('Reason:', style: TextStyle(color: context.colors.txtMuted, fontSize: 12, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 4),
                            Text(leave.reason, style: TextStyle(color: context.colors.txtSec, fontSize: 14)),
                      ...
                      ).animate().fadeIn(delay: (50 * index).ms).slideX(begin: 0.1, end: 0, duration: 400.ms, curve: Curves.easeOut);
                    },
