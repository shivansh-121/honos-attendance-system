...
class _ExecutiveDashboardScreenState extends ConsumerState<ExecutiveDashboardScreen> {
  ...
  void _showPhotoPrompt(BuildContext context, AppUser user) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Profile Picture Required', style: TextStyle(color: NLTheme.primaryText)),
        content: const Text('You must add a profile picture before you can check-in or check-out.', style: TextStyle(color: NLTheme.secondaryText)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          ElevatedButton(
            style: ElevatedButton.styleFrom(foregroundColor: Colors.white, backgroundColor: NLTheme.sidebar, ),
            onPressed: () {
              Navigator.pop(ctx);
              _uploadPhoto(user);
            },
  ...
  void _handleExecutiveAttendance(BuildContext context, AppUser user, bool isCheckOut) {
    ...
    if (Platform.isWindows) {
      showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Mobile Feature Only'),
          content: const Text('Facial recognition and GPS tracking require a mobile device. Please use your phone to mark attendance.'),
          actions: [
      ...
      body: Center(
        child: ConstrainedBox(
          ...
          children: [
            ...
            Row(
              children: [
                Text('EXECUTIVE DASHBOARD', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: context.colors.txtSec, letterSpacing: 1)),
              ],
            ...
            Row(
              children: [
                Expanded(
                  child: _buildAttendanceButton('Check In', Icons.login, NLTheme.accentGreen, () => _handleExecutiveAttendance(context, user, false)),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _buildAttendanceButton('Check Out', Icons.logout, NLTheme.accentPink, () => _handleExecutiveAttendance(context, user, true)),
                ),
              ],
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: _buildStatSquare('Total\nSites', guardsAsync.value?.map((g) => g.siteId).toSet().length.toString() ?? '-', 'Managed', context.colors.primary.withValues(alpha: 0.1)).animate().fadeIn(delay: 400.ms)),
              ],
            ),
            ...
            GridView.count(
              ...
              crossAxisSpacing: 12,
              mainAxisSpacing: 12,
              childAspectRatio: MediaQuery.of(context).size.width > 600 ? 1.5 : 1.1,
              children: [
            ...
            GridView.count(
              ...
              children: [
                ...
                _buildActionButton(title: 'Tracker', icon: Icons.location_on, color: context.colors.bgSurface, textColor: context.colors.txtPrimary, onTap: () => AppNav.push(context, const SupervisorTrackerScreen())),
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text('MANUAL ATTENDANCE', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 1)),
                    const SizedBox(height: 4),
                    const Text('Manually check in/out staff without face recognition', style: TextStyle(color: Colors.white70, fontSize: 12)),
                  ],
                ),
              ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('EXECUTIVE', style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 10, letterSpacing: 1)),
                        ],
                      ),
                    )
                  ],
